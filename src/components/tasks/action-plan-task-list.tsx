        ...
        <div key={group.title} className='mb-8'>
          ...
          <div className='space-y-4 pl-2'>
            {group.tasks.map((task: any) => {
              const realTask = (task.id && tasksById[task.id]) || (task.personId && tasksById[task.personId]);
              return realTask ? (
                <TaskRenderer key={task.id || task.personId} task={realTask} />
              ) : (
                <div key={task.id || task.personId} className='text-xs text-red-400'>
                  Task not found
                </div>
              );
            })}
